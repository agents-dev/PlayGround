...
function damageMinion(min, n) {
  if (min.divine && n > 0) { min.divine = false; log('sys', `✨ ${min.name}'s Divine Shield pops!`); return; }
  min.hp -= n;
}
...
function doBattlecry(side, m, target) {
  ...
  if (m.cardId === 'cleric') {
    ...
    if (!t) { // no target? hit random enemy
      if (G[foe].board.length) { const v = rnd(G[foe].board); damageMinion(v, 3); fxFloat('enemy', v.uid, '-3', '#f66'); }
      else damageHero(foe, 3);
    } else if (t.kind==='hero') damageHero(t.side, 3);
    else damageMinion(t.min, 3);
...
function doSpell(side, def, target, idx) {
  ...
  switch (def.spell) {
    ...
    case 'firelands':
      hurt(T, 5);
      if (me.board.length < 7) { const g = mkMinion('golemtok', side); g.canAttack=false; me.board.push(g); log('sys','🪨 A Stone Golem rises through the portal!'); }
      break;
  }
}
