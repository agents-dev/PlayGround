...
// impact particles pool (single Points, 400 verts)
const PMAX = 500;
const pGeo = new THREE.BufferGeometry();
const pPos = new Float32Array(PMAX * 3), pVel = new Float32Array(PMAX * 3), pLife = new Float32Array(PMAX);
pGeo.setAttribute('position', new THREE.BufferAttribute(pPos, 3));
const points = new THREE.Points(pGeo, new THREE.PointsMaterial({ color: 0xffcc88, size: 0.09, transparent: true, opacity: 0.95, depthWrite: false }));
points.frustumCulled = false; scene.add(points);
let pCursor = 0;
function burst(p, n, color, speed = 5) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    pVel[k * 3] = Math.sin(ph) * Math.cos(th) * speed * Math.random();
    pVel[k * 3 + 1] = Math.abs(Math.cos(ph)) * speed * Math.random() + 1;
    pVel[k * 3 + 2] = Math.sin(ph) * Math.sin(th) * speed * Math.random();
    pLife[k] = 0.5 + Math.random() * 0.4;
  }
}
// decals
const decals = [];
function addDecal(p, n) {
  const d = new THREE.Mesh(new THREE.CircleGeometry(0.09 + Math.random() * 0.06, 10),
    new THREE.MeshBasicMaterial({ color: 0x14100b, transparent: true, opacity: 0.8, polygonOffset: true, polygonOffsetFactor: -2 }));
  d.position.copy(p).addScaledVector(n, 0.02);
  d.lookAt(p.clone().add(n));
  scene.add(d); decals.push({ m: d, life: 25 });
...
function animate() {
  ...
  for (let i = 0; i < PMAX; i++) {
    if (pLife[i] > 0) {
      ...
      if (pPos[i * 3 + 1] < 0.02) { pPos[i * 3 + 1] = 0.02; pVel[i * 3 + 1] = 0; }
    }
  }
...
function updatePlayer(dt, now) {
  ...
  // fire
  if (state.firing && now >= state.nextShot && !state.reloadingUntil && !state.matchOver) {
    playerShoot(now);
...
// test hooks
window.__game = { state, bots, THREE, ready: true };
